...
class FileNaming:
    ...
    def remove_reserved_filename(self, old_filename: str) -> None:
        ...
        """
        dictionary_of_files: dict[
            BaseFile, dict[str, BaseFile]
        ] = self.reserved_index.get(old_filename, {})
        reference: dict[str, BaseFile] | None = dictionary_of_files.get(
            self.related_file_object, None
        )
    ...
    def rename(self) -> None:
        ...
        if save_to is None or not complete_filename:
            raise ImproperlyConfiguredFile(
                "Renaming a file without a directory set at `save_to` and without a "
        ...
        if (
            reserved_folder
            and object_reserved
            and object_reserved is not self.related_file_object
        ):
            if not self.on_conflict_rename:
                raise ReservedFilenameError(
                    f"Rename cannot be made, because the filename {complete_filename} is "
                ...
                reserved_names: list[str] = [
                    filename for filename in reserved_folder.keys()
                ]
                ...
                self.related_file_object.rename_pipeline.run(
                    ...
                    **{
                        **self.related_file_object._get_kwargs_for_pipeline(
                            "rename_pipeline"
                        ),
                ...
                # `save` method.
                self.related_file_object.hashes.rename(
                    self.related_file_object.complete_filename
                )

        # Update reserved dictionary to reserve current filename.
        if not reserved_folder:
            self.reserved_filenames[save_to] = {
                complete_filename: self.related_file_object
            }
        ...
        if complete_filename not in self.reserved_index:
            ...
            self.reserved_index[complete_filename] = {
                self.related_file_object: self.reserved_filenames[save_to]
            }
        else:
            self.reserved_index[complete_filename][
                self.related_file_object
